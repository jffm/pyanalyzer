...
class AvoidRoutineWithTooManyParameters(Rule):
    """Too many parameters (%(parameters)s/%(max)s) for routine %(name)s
    
    Rationale: A function with too many parameters is difficult to use.
    Note that in Python, one could use keywords arguments **kwargs. But this
    could cause a warning in pylint
    """
    class config:
        severity = "convention"
        code = 201
    ...
    def visitFunction(self, node, *args):
        parameters = node.NumberOfParameters
        if parameters > self.config.max_parameters:
            self.report(
                node,
                dict(parameters=parameters, max=max_parameters, name=node.name)
            )
