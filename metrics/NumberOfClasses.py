...
class CountDeclaredClasses(Visitor):
    result = 0
    def visitClass(self, node, *args):
        self.result += 1

class NumberOfClasses(Metric):
    """Number of (inner/local)classes of a (Project, Module, Class, Function)
    """
    def countClasses(self, node, *args):
        node.NumberOfClasses = compiler.walk(node, CountDeclaredClasses()).result
    visitFunction = countClasses
    def visitClass(self, node, *args):
        # don't count itself as an inner class, hence start at node.code
        self.countClasses(node.code, *args)
        self.default(node, *args)
    def visitProject(self, node, *args):
        self.countClasses(node, *args)
        self.default(node, *args)
    visitFunction = visitModule = visitProject
