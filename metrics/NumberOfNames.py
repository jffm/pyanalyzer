...
class NamesCounter(Visitor):
    result = 0
    def visitName(self, node, *args):
        self.result += 1

class NumberOfNames(Metric):
    """Number of names used in a (Project, Module, Class, Function)
    """
    def countNames(self, node, *args):
        node.NumberOfNames = compiler.walk(node, NamesCounter()).result
    visitFunction = countNames
    def visitProject(self, node, *args):
        self.countNames(node, *args)
