...
class NumberOfAttributes(Metric):
    """Number of attributes of a (Class)
    This is computed by the accesses to self.* that are not methods
    """
    requires = ('InternalFieldAccesses',)
    def visitClass(self, node, *args):
        node.NumberOfAttributes = len(node.InternalFieldAccesses)