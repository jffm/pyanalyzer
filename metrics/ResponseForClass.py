...
class ResponseForClass(Metric):
    ...
    that class is invoked for that object. In practice a rough approximation
    is calculated by inspecting method calls within the class's method bodies.
    
        ResponseForClass(Class) :=
            Sum(NumberOfCalls(Method) forall Method in Class)
            +
            NumberOfFunctions(Class)
    """
    requires = ('NumberOfCalls', 'NumberOfFunctions')
    def visitClass(self, node, *args):
        try:
            node.ResponseForClass = node.NumberOfCalls + node.NumberOfFunctions
