...
class DocumentationLinesCounter(Visitor):
    result = 0
    def visitModule(self, node, *args):
        self.result += node.doc and node.doc.count("\n") or 0
        self.default(node, *args)
    visitClass = visitFunction = visitModule

class NumberOfLinesOfDocumentation(Metric):
    """Number of lines of documentation of a (Module, Class, Function)
    
    Count the number of lines in the docstring of the code element
    """
    def visitProject(self, node, *args):
        node.NumberOfLinesOfDocumentation = compiler.walk(node, DocumentationLinesCounter()).result
        self.default(node, *args)
    def countDocumentationLines(self, node, *args):
        node.NumberOfLinesOfDocumentation = node.doc and node.doc.count("\n") or 0
    visitFunction = countDocumentationLines
    def visitModule(self, node, *args):
        self.countDocumentationLines(node, *args)
        self.default(node, *args)
