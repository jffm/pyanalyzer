...
class CountDeclaredImports(Visitor):
    result = 0
    def visitImport(self, node, *args):
        self.result += len(node.names)
        
class NumberOfImports(Metric):
    """Number of imported modules of a (Project, Module)
    """

    def visitProject(self, node, *args):
        node.NumberOfImports = compiler.walk(node, CountDeclaredImports()).result
