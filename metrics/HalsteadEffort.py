...
class HalsteadEffort(Metric):
    """Halstead effort to implement a (Project, Module, Function)
    
    The effort to implement (E) or understand a program is proportional to the
    volume and to the difficulty level of the program.
        E = V * D 
    
    source: http://www.verifysoft.com/en_halstead_metrics.html
    """
    requires = ('HalsteadVolume','HalsteadDifficulty')
    def countHalsteadEffort(self, node, *args):
        node.HalsteadEffort = node.HalsteadVolume * node.HalsteadDifficulty
    visitFunction = countHalsteadEffort
    def visitProject(self, node, *args):
        self.countHalsteadEffort(node)
        self.default(node)
