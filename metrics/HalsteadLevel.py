...
class HalsteadLevel(Metric):
    """Halstead Level of a (Project, Module, Function)
    ...
    """
    requires = ('HalsteadDifficulty',)
    def countHalsteadLevel(self, node, *args):
        node.HalsteadLevel = (
            node.HalsteadDifficulty
            and (1.0/node.HalsteadDifficulty)
            or 1
        )
    visitFunction = countHalsteadLevel
    def visitProject(self, node, *args):
        self.countHalsteadLevel(node)
        self.default(node)
