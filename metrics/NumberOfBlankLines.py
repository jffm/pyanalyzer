...
class BlankLinesCounter(Visitor):
    result = 0
    def countBlankLines(self, node, *args):
        self.result += len(node.blanklines)
        self.default(node, *args)
    visitClass = visitFunction = visitModule = countBlankLines

class NumberOfBlankLines(Metric):
    """Number of blank lines in a (Module, Class, Function)
    """
    def visitProject(self, node, *args):
        node.NumberOfBlankLines = compiler.walk(node, BlankLinesCounter()).result
        self.default(node, *args)
    def visitModule(self, node, *args):
        node.NumberOfBlankLines = len(node.blanklines)
        self.default(node, *args)
