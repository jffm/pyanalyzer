...
class ClassReferencesGetter(Visitor):
    def __init__(self):
    ...
    def visitName(self, node, *args):
        if (hasattr(node, "reference")
            ...
            and ("Class" in node.reference.kind)):
            self.result.add(node.reference)
    visitGetattr = visitName

class ClassReferences(Metric):
    """List of classes referenced by a (Module, Class, Function)
    """
    resolve = True # Name resolving needed
    report = False
    def visitModule(self, node, *args):
        node.ClassReferences = compiler.walk(node, ClassReferencesGetter()).result
