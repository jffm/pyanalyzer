...
class NumberOfReferencedClasses(Metric):
    ...
    requires = ('ClassReferences',)
    def visitModule(self, node, *args):
        node.NumberOfReferencedClasses = len(
            set(
                reference.class_ 
                for reference in node.ClassReferences 
                    if ("Class" in reference.kind)
                    and (reference.module != node)
            )
        )
    ...
    def visitFunction(self, node, *args):
        node.NumberOfReferencedClasses = len(
            set(
                ...
                for reference in node.ClassReferences 
                    if ("Class" in reference.kind)
                    and (reference.class_ != node.class_)
            )
        )