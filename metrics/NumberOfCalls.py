...
class CallsCounter(Visitor):
    result = 0
    def visitCallFunc(self, node, *args):
        self.result += 1
        self.default(node)

class NumberOfCalls(Metric):
    """Number of calls performed in a (Project, Module, Class, Function)
    """
    def countCalls(self, node, *args):
        node.NumberOfCalls = compiler.walk(node, CallsCounter()).result
    visitFunction = countCalls
    def visitProject(self, node, *args):
        self.countCalls(node, *args)
