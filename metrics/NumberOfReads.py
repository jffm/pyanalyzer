...
class FieldReadsCounter(Visitor):
    result = 0
    def visitGetattr(self, node, *args):
        self.default(node, *args)
        self.result += 1

class NumberOfReads(Metric):
    """Number of reads of a (Project, Module, Class, Function)
    
    Count the number of read accesses to fields
        object.field
    """

    def countFieldReads(self, node, *args):
        node.NumberOfReads = compiler.walk(node, FieldReadsCounter()).result
    visitFunction = countFieldReads
    def visitProject(self, node, *args):
        self.countFieldReads(node, *args)
        self.default(node, *args)
