...
class PublicFunctionsCounter(Visitor):
    result = 0
    def visitFunction(self, node, *args):
        if not node.name.startswith("_"):
            self.result += 1

class NumberOfPublicFunctions(Metric):
    """Number of public functions of a (Project, Module, Class)
    ...
    def countFunctions(self, node, *args):
        node.NumberOfPublicFunctions = compiler.walk(node, PublicFunctionsCounter()).result
