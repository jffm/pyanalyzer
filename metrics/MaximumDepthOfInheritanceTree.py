...
def maximum_depth_of_inheritance_tree(Class):
    if Class.inherits:
        return (max(
            maximum_depth_of_inheritance_tree(parent)
            for parent in Class.inherits
        )+1)
    else:
        return 0

class MaximumDepthOfInheritanceTree(Metric):
    """Maximum depth of the inheritance tree of a (Class)
    
    """
    resolve = True # Name resolving needed
    def visitClass(self, node, *args):
        node.MaximumDepthOfInheritanceTree = maximum_depth_of_inheritance_tree(node)