...
class CsvWriter(TextWriter):
    def __init__(self, fileobject, fields):
        self.writer = csv.writer(fileobject, delimiter=",")
        self.fields = fields
        self.writer.writerow([field for (field, type) in fields])
    def writerow(self, row):
        self.writer.writerow([row[field] for (field, type) in self.fields])
