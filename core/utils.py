...
"""Utilities for the pyanalyzer.core package
"""
...
def log2(n):
    return log(n or 1)/log(2) # to prevent OverflowError due to log(0)

def extract(text, delim=",", line_continuation="\\"):
    """Extracts parts separated by @delim from @text
    This function is used to read values of options written in 
    """
    return [
        part.strip()
        for part in text.replace(line_continuation,"").split(delim)
    ]